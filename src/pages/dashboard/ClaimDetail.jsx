...
export default function ClaimDetail() {
  ...
  const stepIdx = getProgressStepIndex(claim.status)
  ...
  const getProgressSteps = () => {
    ...
    if (claim.status === 'rejected') {
      const rejectEvent = claim.document_request_history?.find(item => item.type === 'reaction' && item.label === 'Claim Rejected');
      if (rejectEvent) {
        const roleLabel = rejectEvent.by === 'adjuster' ? 'Adjuster'
          : rejectEvent.by === 'siu_investigator' ? 'SIU Investigator'
            : rejectEvent.by === 'supervisor' ? 'Supervisor'
              : rejectEvent.by === 'admin' ? 'Administrator'
                : rejectEvent.by;
        rejectDetails = {
          actor: roleLabel,
          time: rejectEvent.date,
          notes: rejectEvent.message
        };
      } else {
        rejectDetails = {
          actor: 'SIU Investigator',
          time: claim.closed_at || claim.updated_at,
    ...
    const steps = [
      ...
      {
        title: claim.status === 'rejected' ? `Claim Rejected by ${rejectDetails.actor}` : 'Final Settlement / Resolution',
        description: claim.status === 'rejected'
          ? `Notes: "${rejectDetails.notes}"`
          : 'Settling or closing the claim, and transferring approved payout to bank account.',
        actor: claim.status === 'rejected' ? `${rejectDetails.actor}`
          : claim.status === 'settled' && claim.auto_settle_eligible ? 'AI Settlement Agent (A6)'
            : currentStepIdx === 5 ? 'Human Adjuster / Supervisor'
              : 'AI Settlement Agent (A6)',
        time: claim.status === 'rejected' ? rejectDetails.time : (claim.closed_at || (currentStepIdx === 5 ? claim.updated_at : null))
      }
    ]
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}>
      {/* Back & Track Progress */}
      <motion.div variants={{ hidden: { y: 20 }, visible: { y: 0 } }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
      ...
      {/* Header */}
      <motion.div variants={{ hidden: { y: 20 }, visible: { y: 0 } }} className="page-heading">
      ...
      {/* Info cards */}
      <motion.div variants={{ hidden: { y: 20 }, visible: { y: 0 } }} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 16, marginBottom: 24 }}>
        {[
      ...
      <motion.div variants={{ hidden: { y: 20 }, visible: { y: 0 } }} style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card">
          {PIPELINE_STEPS.map((step, i) => {
            ...
            const active = isClaimFinalized ? false : i === stepIdx
            const isRejectedStep = claim.status === 'rejected' && step === 'settled'
            
            let label = step === 'damage_assessment'
            ...
            return (
              <div key={step} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: i < PIPELINE_STEPS.length - 1 ? 12 : 0 }}>
                <div style={{
                  ...
                  background: isRejectedStep ? 'rgba(239, 68, 68, 0.2)' : done ? 'rgba(16, 185, 129, 0.2)' : active ? 'rgba(79, 70, 229, 0.2)' : 'rgba(255, 255, 255, 0.05)',
                  border: `2px solid ${isRejectedStep ? '#EF4444' : done ? '#10B981' : active ? '#818CF8' : 'rgba(255, 255, 255, 0.1)'}`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '0.75rem', fontWeight: 700,
                  color: isRejectedStep ? '#EF4444' : done ? '#10B981' : active ? '#818CF8' : 'var(--text-dim)',
                }}>
                  {isRejectedStep ? '✗' : done ? '✓' : i + 1}
                </div>
                <span style={{
                  fontSize: '0.88rem',
                  fontWeight: active || isRejectedStep || (done && i === PIPELINE_STEPS.length - 1) ? 600 : 400,
                  color: isRejectedStep ? '#EF4444' : done ? 'var(--text-muted)' : active ? 'var(--text)' : 'var(--text-dim)',
                }}>{label}</span>
                {isRejectedStep && <span style={{ marginLeft: 'auto', fontSize: '0.75rem', color: '#EF4444', fontWeight: 600 }}>Rejected</span>}
                {!isRejectedStep && done && <span style={{ marginLeft: 'auto', fontSize: '0.75rem', color: 'var(--success)' }}>Done</span>}
          ...
          {showTrace && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {trace.map((step, i) => {
                ...
                return (
                  <motion.div key={i} variants={{ hidden: { y: 10 }, visible: { y: 0 } }} style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', position: 'relative' }}>
                  {getProgressSteps().map((step, idx, arr) => {
                    ...
                    const isRejectedNode = claim.status === 'rejected' && idx === 5
                    const isApprovedNode = claim.status === 'settled' && idx === 5
                    ...
                    return (
                      <div key={idx} style={{ display: 'flex', gap: 16, position: 'relative', marginBottom: idx < arr.length - 1 ? 24 : 0 }}>
                        ...
                        <div style={{
                          ...
                          height: 30,
                          borderRadius: '50%',
                          background: isRejectedNode ? 'rgba(239, 68, 68, 0.15)'
                                      : isCompleted ? 'rgba(16, 185, 129, 0.15)' 
                                      : 'rgba(255, 255, 255, 0.03)',
                          border: `2px solid ${isRejectedNode ? '#EF4444' : isCompleted ? '#10B981' : 'rgba(255, 255, 255, 0.12)'}`,
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          zIndex: 2,
                          flexShrink: 0,
                          color: isRejectedNode ? '#EF4444' : isCompleted ? '#10B981' : 'var(--text-dim)',
                          fontSize: '0.75rem',
                          fontWeight: 700,
                          boxShadow: isRejectedNode ? '0 0 10px rgba(239, 68, 68, 0.2)'
                                     : isCompleted ? '0 0 10px rgba(16, 185, 129, 0.2)' 
                                     : 'none'
                        }}>
                          {isRejectedNode ? '✗' : isApprovedNode ? '✓' : idx < getProgressStepIndex(claim.status) ? '✓' : idx + 1}
                        </div>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 6 }}>
                            ...
                            <span style={{
                              ...
                              padding: '2px 8px',
                              borderRadius: 4,
                              background: isRejectedNode ? 'rgba(239, 68, 68, 0.12)'
                                        : isApprovedNode ? 'rgba(16, 185, 129, 0.12)'
                                        : step.active ? 'rgba(99,102,241,0.12)' 
                                        : idx < getProgressStepIndex(claim.status) ? 'rgba(16,185,129,0.12)' 
                                        : 'rgba(255,255,255,0.03)',
                              color: isRejectedNode ? '#EF4444'
                                   : isApprovedNode ? '#10B981'
                                   : step.active ? 'var(--accent)'
                                   : idx < getProgressStepIndex(claim.status) ? '#10B981'
                                   : 'var(--text-dim)',
                            }}>
                              {isRejectedNode ? 'Rejected' 
                               : isApprovedNode ? 'Approved'
                               : step.active ? 'Active' 
                               : idx < getProgressStepIndex(claim.status) ? 'Completed' 
                               : 'Pending'}
                            </span>
                          </div>
                                ...
                                {claim.document_request_history.map((item, itemIdx) => {
                                  ...
                                  const roleLabel = item.by === 'adjuster' ? 'Adjuster'
                                    : item.by === 'siu_investigator' ? 'SIU Investigator'
                                      : item.by === 'supervisor' ? 'Supervisor'
                                        : item.by === 'admin' ? 'Administrator'
                                          : item.by === 'system' ? 'System'
                                            : item.by;

                                  const badgeColor = isRequest ? '#EF4444'
                                    : isReaction ? '#818CF8'
                                      : '#10B981';
                                  const bgColor = isRequest ? 'rgba(239, 68, 68, 0.08)'
                                    : isReaction ? 'rgba(99, 102, 241, 0.08)'
                                      : 'rgba(16, 185, 129, 0.08)';
                                  const borderColor = isRequest ? 'rgba(239, 68, 68, 0.2)'
                                    : isReaction ? 'rgba(99, 102, 241, 0.2)'
                                      : 'rgba(16, 185, 129, 0.2)';
                                  const title = isRequest
                                    ? `Document Request #${item.sequence} by ${roleLabel}`
                                    : isReaction
                                      ? `${item.label} by ${roleLabel}`
                                  ...
                                  return (
                                    <div key={itemIdx} style={{
                                      ...
                                      <p style={{ margin: 0, color: 'var(--text-muted)', lineHeight: 1.4, fontSize: '0.75rem' }}>
                                        {isRequest ? `Message: "${item.message}"`
                                          : isReaction ? `Notes: "${item.message}"`
                                            : `Uploaded Files: ${item.message}`}
                                      </p>
                                    </div>
                                  );
                                })}
